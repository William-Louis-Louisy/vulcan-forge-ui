...
import { generateReactNativeThemeExport } from './react-native-theme-export';
...
describe('generateReactNativeThemeExport', () => {
  it('generates a React Native theme file from resolved design tokens', () => {
    const result = generateReactNativeThemeExport({
      projectName: 'Aurora System',
      tokens,
    });

    expect(result.fileName).toBe('aurora-system-react-native-theme.ts');

    expect(result.content).toContain('// Aurora System — React Native theme');
    expect(result.content).toContain('export const tokens =');
    expect(result.content).toContain('export const themes =');
    expect(result.content).toContain('export const lightTheme =');
    expect(result.content).toContain('export const darkTheme =');
    expect(result.content).toContain('export function getTheme');
    expect(result.content).toContain('export const reactNativeTheme =');
    expect(result.content).toContain(
      'export type ReactNativeTokens = typeof tokens;',
    );
    expect(result.content).toContain('export default reactNativeTheme;');
